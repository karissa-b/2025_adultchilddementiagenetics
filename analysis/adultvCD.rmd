...
genesets$adult_all <- 
  read_csv("data/genesets/Alternate_gene_lists_for_NetColoc_senstivity_analyses/adult_all.csv")$GENE


genesets.bystype <- list.files(
...
Only 13 genes are shared between adult- and childhood-onset dementia
```{r}
genesets %>%
  .[
    grepl(names(genesets), pattern = "CD|adult|brain")
  ] %>% 
...
genesets[names(genesets) %>% grepl(pattern = "_all")] %>% 
...
kegg[names(kegg) %>% grepl(pattern = "all")] %>% 
  lapply(function(x) {x$Description}) %>% 
  fromList() %>% 
  UpSetR::upset()
...
<!-- reactome[names(reactome) %>% grepl(pattern = "all")] %>%  -->
<!--     lapply(function(x) {x$Description}) %>%  -->
<!--   fromList() %>%  -->
<!--   UpSetR::upset( -->
<!--     order.by = "freq",  -->
...
enrichment_dfs <- 
  list.files(
    "data/results/adultchild/CD_all_adult_allv2", 
...
enrichment_dfs %>% 
  ...
  geom_errorbar(
    ...
    #colour = "white",
    aes(xmin = value - 2 * expected_overlap_std,
        xmax = value + 2 * expected_overlap_std),
    width = 0.25
  ) +
  geom_label(
    data =  . %>% 
    ...
    aes(y = 2.5, x = 190, 
        label = sprintf("p = %.1e", empirical_p)),
    inherit.aes = FALSE,
    #colour = "white",
    fill = NA,
    size = 2.5,
  ...
  scale_fill_manual(values = c("grey50", "skyblue"))+
   ...
   # ggsave("output/plots4poster/networkcoloc_concatgenesets.svg", 
...
gtex %<>% 
  mutate(
    tissue = sub(tissue, pattern = "_.*$", replacement = "")
  ) %>% 
  dplyr::distinct(gene, tissue, .keep_all = TRUE)
...
genesets %>% 
  ...
  # ggsave(
  #   "output/adult v child/figs/talk/numgenespergs.svg", 
...
p1 <-  genesets %>%
  ...
  unnest(gene) %>%
  mutate(present = 1) %>% 
  dplyr::distinct(gene, geneset,  .keep_all = TRUE) %>%
  pivot_wider(
    names_from = geneset,
    values_from = present,
  ) %>%
  mutate(across(-gene, ~replace_na(., 0))) %>%
  mutate(across(-gene, as.logical)) %>% 
  simpleUpSet(
    sets = c("adult_all", "brain_malform", "CD_all"), 
    min_size = 1,
    width = c(0.4,0.6),
    height = c(0.8, 0.2),
    n_intersect = 20,
    ...
    plot_annotation(title = "Overlap of genes")
...
p2 <- kegg %>% 
  ...
  plot_annotation(
    title = "Overlap of KEGG pathways")

ggarrange(p1, p2, labels = "AUTO")

  ggsave(
    "output/figs/paper/overlap-plots.svg", 
    width = 6, height = 5, scale = 5, units = "cm", 
  )
```
...
comparison_names = list(
  CD_all.adult_all = "childhood dementia\n\ & \nadult-onset NDD", 
  AD_PD = "Alzheimer's disease\n&\nParkinson's disease",
  CD_all_brain_malform = "childhood dementia\n&\nbrain malformations"
)
...
enrichment_dfs %>% 
  mutate(
    comparison = str_replace(
      comparison, 
      pattern = "CD_all_adult_all", 
      replacement = comparison_names$CD_all.adult_all
    ), 
    comparison = str_replace(
      comparison, 
      pattern = "CD_all_brain_malform", 
      replacement = comparison_names$CD_all_brain_malform
    ),
    comparison = str_replace(
      comparison, 
      pattern = "AD_PD", 
      replacement = comparison_names$AD_PD
    ),
    comparison = factor(comparison, 
                        levels = c(comparison_names$AD_PD, 
                                   comparison_names$CD_all_brain_malform,
                                   comparison_names$CD_all.adult_all
  ...
  geom_text(
    data =  . %>% 
      filter(type == "expected"),
    aes(x = 1.5, y = 350, label = sprintf("p = %.2e", empirical_p)),
    inherit.aes = FALSE,
    vjust = 0,
    hjust = 0.5,
    fontface = "bold",
  ...
  theme(legend.position = "none") +
  scale_fill_manual(values = c("grey50", "skyblue")) -> p3

node_metadata$CD_all_adult_all %>% 
  ggplot(
    aes(x = z_d1, y = z_d2, )
  ) +
  geom_point(
  ...
  geom_label_repel(
    aes(label = X), 
    data = . %>% 
      dplyr::filter(z_both > 100)
  ) +
  scale_color_viridis_c() +
  labs(
    x = expression(italic(z[phantom(.) * "child"])),
    y = expression(italic(z[phantom(.) * "adult"])),
    title = 
    ) + 
  theme(
 ...
 p3 + p4 +
  ...
  ggsave(
    "output/figs/paper/netcoloc.svg", 
    width = 7, height = 2.5, units = "cm", 
    scale = 4)
```

## number genes per subnet
...
node_metadata %>% 
  ...
  dplyr::count(seed_type, comparison) %>% 
  ggplot(aes(x = "", y = n, fill = seed_type)) +
  ...
  labs(
    title = "Proportion of Genes by Seed Type",
    fill = "Seed Type"
  ) +
  theme_void() +
  facet_wrap(~comparison, scales = "free") +
  ggsave(
       "output/adult v child/figs/talk/subnetwork_ngenes_piechart.svg", 
    width = 5, height = 3, units = "cm", 
    scale = 4)
```

## Are the subnetworks brain expressed?
...
list.files(
 ...
 dplyr::filter(comparison != "CD_all_brain_malform") %>%
  ...
  facet_wrap(~comparison, scales = "free") +
   ggsave(
    "output/adult v child/figs/talk/MPO_enrichment.svg", 
...
cluster.gprofiler.results[c("AD_PD", "CD_all_adult_all")] %>% 
  ...
  ggpubr::ggarrange(
    plotlist = .) +
     ggsave(
    "output/adult v child/figs/talk/cluster_enrichment.svg", 
    width = 10, height = 3, units = "cm", 
    scale = 4)
```
```
