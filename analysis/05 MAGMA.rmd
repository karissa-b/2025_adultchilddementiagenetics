...
p.topSNP.GSA +
  ggsave("output/plot44invest.png", 
         height = 6, width = 10,
         units = "cm", scale = 2)
```
...
```{r}
topSNP.GSA %>% 
  dplyr::filter(type == "Seed genes only") %>% 
   arrange(pvalue) %>% 
  mutate(
    Disease = fct_inorder(Disease) %>% fct_rev(),
    type = fct_rev(type)
  ...
  ggplot(
    aes(x =  -log10(pvalue), y = Disease, fill = Disorder)
  ) +
  geom_col(
    position = "dodge",
    colour = "black")  + 
...
list(
  MAGMA_top = CD_merged, 
  MAGMA_multi = CD_merged.multi, 
  MAGMA_GSA_top = topSNP.GSA %>% dplyr::select(
    CD = Disease, 
    adult = Disorder, type, pvalue, qvalue), 
  MAGMA_GSA_multi = multi.GSA %>% dplyr::select(
    CD = Disease, 
    adult = Disorder, type, pvalue, qvalue)
  ) %>% 
  writexl::write_xlsx(
    "output/spreadsheets/TableS4 MAGMA.xlsx"
  )
```
